...
class ConfigWindow(QtWidgets.QMainWindow):
    ...
    def __init__(self):
        ...
        self.hotkey_status_label.setAlignment(QtCore.Qt.AlignRight | QtCore.Qt.AlignVCenter)
        font2 = QtGui.QFont()
        ...
        # —— 9. 初始化样式——第二次调用 on_style_changed，以展示初始示例浮窗 —— #
    ...
    def on_auto_detect_model(self):
        ...
        QtWidgets.QMessageBox.information(self, "提示", f"已自动检测并设置模型：{default}")
    ...
    def _update_autostart(self, enable: bool):
        ...
        if enable:
            ...
            try:
                ...
                QtWidgets.QMessageBox.warning(self, "开机自启", f"创建启动脚本失败：{e}")
        else:
            try:
                if os.path.exists(autostart_path):
                    os.remove(autostart_path)
            except Exception as e:
                QtWidgets.QMessageBox.warning(self, "开机自启", f"删除启动脚本失败：{e}")
